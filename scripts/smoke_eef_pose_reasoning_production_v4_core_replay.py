...
REPLAY_PUBLICATION_FIX_COMMIT = "d32115d36f2dea510dee86edeaddcc58309afc2e"
...
def production_repository_provenance(expected_commit: str) -> dict[str, Any]:
    ...
    publication_fix = subprocess.run(
        ["git", "rev-parse", "HEAD^"],
        cwd=repository["path"],
        check=True,
        capture_output=True,
    ...
    require(
        publication_fix == REPLAY_PUBLICATION_FIX_COMMIT,
        "production replay publication-fix parent drift",
    )
    require(
        validation_fix == REPLAY_VALIDATION_FIX_COMMIT,
        "production replay validation-fix grandparent drift",
    )
    require(
        implementation == REPLAY_IMPLEMENTATION_COMMIT,
        "production replay implementation great-grandparent drift",
    )
    require(
        parent == REPLAY_PARENT_COMMIT,
        "production replay test great-great-grandparent drift",
    )
    require(
        production_base == PRODUCTION_BASE_COMMIT,
        "production replay core great-great-great-grandparent drift",
    )
    return {
        **repository,
        "replay_publication_fix_commit": REPLAY_PUBLICATION_FIX_COMMIT,
