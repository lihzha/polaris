...
[[ "${FULLTRACE_CONTAINER_SHA256}" =~ ^[0-9a-f]{64}$ ]]
[[ "${SLURM_NTASKS:-}" == 1 ]]
[[ "${SLURM_PROCID:-0}" == 0 ]]
[[ "${SLURM_LOCALID:-0}" == 0 ]]
...
readonly attempt="${FULLTRACE_OUTPUT_ROOT}/${variant}/job_${SLURM_JOB_ID}_restart_${SLURM_RESTART_COUNT:-0}"
readonly cache="${FULLTRACE_HOST_CACHE_ROOT}/${variant}/job_${SLURM_JOB_ID}_restart_${SLURM_RESTART_COUNT:-0}"
...
write_failure_marker() {
  ...
  if [[ "${status}" != 0 && -d "${attempt}" && ! -e "${failure_marker}" ]]; then
...
finish() {
  local rc="$?"
  trap - EXIT
  if [[ "$(git -C "${FULLTRACE_POLARIS_REPO}" rev-parse HEAD)" != "${FULLTRACE_REPLAY_COMMIT}" ]] ||
    [[ -n "$(git -C "${FULLTRACE_POLARIS_REPO}" status --porcelain=v1 --untracked-files=all)" ]]; then
    rc=1
    status=1
  fi
  if [[ "${cache}" == "${FULLTRACE_HOST_CACHE_ROOT}/${variant}/job_"* ]] &&
    [[ -e "${cache}" ]]; then
    rm -rf -- "${cache}" || {
      rc=1
      status=1
...
printf 'PROFILE=%s\nSLURM_JOB_ID=%s\nSLURM_RESTART_COUNT=%s\nVARIANT=%s\nLAUNCH_ID=%s\nREPLAY_COMMIT=%s\nREPLAY_PARENT_COMMIT=%s\nPRODUCTION_BASE_COMMIT=%s\nCONTAINER_IMAGE=%s\nCONTAINER_SHA256=%s\nCONTAINER_PYTHONPATH=%s\nRESULT_JSON=%s\nVIDEO=%s\nMANIFEST=%s\n' \
  ...
  "${replay_parent_commit}" "${production_base_commit}" "${FULLTRACE_CONTAINER_IMAGE}" \
...
printf 'profile=production_v4_core_fulltrace_success_v1\njob_id=%s\nvariant=%s\nresult_sha256=%s\nvideo_sha256=%s\nmanifest_sha256=%s\n' \
