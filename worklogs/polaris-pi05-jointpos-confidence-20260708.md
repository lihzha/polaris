...
- Current setup `1101831` completed `0:0` in 10:27 and historical setup
  `1101832` completed `0:0` in 13:07. Both verified 242 distributions, all
...
- Paired one-rollout canaries `1101835` (current) and `1101836` (historical)
  both passed package, checkpoint, tokenizer, model-runtime v3, serving, GPU,
...
- The current failure log is 19,601 bytes with SHA-256
  `78bca931f646403f3613e893216908f9f826a43b24978dd431c46c2b90dc8d19`.
  The historical control independently produced the same exception; its
  19,737-byte log has SHA-256
...
- Fix commit `61cfddc5773e35ee355eccfdf76cdc3f9aebf1f1` retains exact-set
  rejection, adds only that pinned identity, advances the graphics profile to
  `l401_pyxis_nvidia_580_105_08_mapped_graphics_v2`, and binds canonical
  digest
  `f3ee6c8027f0cfea3c0f4875c2d3c0aba4c8cf41f8bde040a0bf236b81133a84`.
  Current and historical runtime/test blobs are byte-identical at
  `41e679ae754fa2037190ec9613a95c386aca5baf` and
  `c65935ff5da3623bda75236a733e250589ef61e9`. Host validation remains 487
  passed plus eight subtests for current and 183 plus eight for historical;
...
- Shared registry revision 30 records both v11 failures and the v12 prelaunch
  ...
  `/lustre/fsw/portfolios/nvr/users/lzha/src/PolaRiS-pi05-confidence-current-61cfddc-20260708T225010Z`
  is clean at tree `789c5027cd1819a684b773c41e0a06b1ee1316b5`; the paired
  historical checkout is clean at commit
  `449ab6465450a871221803d529f3febb92e5d62b`. CPU-only setup jobs
  `1101841` and `1101842` were submitted under
...
- Fresh setup jobs `1101841` (current) and `1101842` (historical) completed
  `0:0` in 11:22 and 12:39. Both passed 242 distributions, 240 noneditable
  ...
  and clean-source gates. Current sealed/final package canonical SHA-256 is
...
- Current canary `1101847` and historical canary `1101848` ran on independent
  L40S nodes `pool0-00025` and `pool0-00034`. They passed package,
...
- Live Kit logs independently record those exact empty-string overrides on
  ...
  cancellation are invalid. Current final eval/log SHA-256 values are
  `db567b6d...` and `d937973e...`; full identities are preserved in the shared
  evaluation registry.

## 2026-07-08 — graphics profile v3 ready for fresh setup

- Fix commit `52423d3f04949ecf619e8f58a8912299d4f1c2ec` requires the
  five Vulkan variables that Kit explicitly clears (`VK_SDK_PATH`,
  `VULKAN_SDK`, `VK_LAYER_PATH`, `VK_INSTANCE_LAYERS`, and
...
- Current and historical runtime/test blobs remain byte-identical at
  `ea00ae58a0674306120f010a2010d1bc8c460e01` and
  `734d8feb4931641f94a1f6ce2254825bec19212c`. Current validation is 495
  passed plus eight subtests; historical is 191 plus eight; focused
  runtime/evidence/serving validation is 97/97 on each arm. Fresh frozen v13
  checkouts are clean at current commit/tree
  `52423d3f04949ecf619e8f58a8912299d4f1c2ec`/
  `fcd31b611db6188dae28f31ee3633c637b5642cf` and historical commit/tree
  `a9ce3af10440528f020a92f7e3cbc2743a909702`/
  `a31ae20d91e0bed34a6a8e505160610852340919`. Fresh setup remains the next
